...
class MCTS_BASE():
    ...
    def play(self, root, idx :int, file_path :str, alternatives: int = 1, train: bool = True, 
        ...
        for iteration in range(self.max_route_len):
            ...
            if (self.debug):
                ...
                if self.debug: print("search_space:")
                for item in self.search_space:
                    print(item,":",self.search_space[item])
                if self.debug: print("routelist:")
                for item in routelist:
                    print(item)
                if self.debug: print("finished routelist:")
        ...
        with open(save_path, 'w+') as f:
            ...
            if self.debug: print(f"成功保存训练数据到：{os.path.abspath(save_path)}")  # 打印绝对路径确认
        return ret
    ...
    def _dfs_main(self, root):  
        ...
        if root not in self.search_space:
            ...
            if self.debug: print(f"         Total branches: {len(node.childlist)}")
            for i, (childs, p) in enumerate(zip(node.childlist, node.Plist)):
                # 获取初始价值（用于调试）
                v_init = self.gen_value(childs[0]) if childs else -1  # 取第一个前体估计
                if self.debug: print(f"  Branch {i}: {childs} | P={p:.3f} | V_init≈{v_init:.3f}")
        # =========================
        ...
        for i in range(self.search_space[root].choice_len):
            ...
            if self.debug: print(f"  Branch {i}: N={n:3d} | Q={q:.3f} | P={p:.3f} | Policy={policy[i]:.3f}")
        # =====================
    ...
    def _dfs(self, root: str, depth: int, ancestors): 
        ...
        if self.debug: print(f"      {'Idx':<3} {'Precursors':<30} {'P':<6} {'N':<4} {'Q':<6} {'U':<6} {'PUCT':<6}")
        if self.debug: print(f"      {'-'*60}")
        ...
        for i in range(self.search_space[root].choice_len):
            ...
            if self._check_cycle(childs, ancestors): 
                if self.debug: print(f"      {i:<3} {'(cycle) ' + '.'.join(childs):<30} {p:<6.3f} {n_sa:<4} {q:<6.3f} {'-':<6} {'-':<6}")
                continue
            ...
            if self.debug: print(f"      {i:<3} {precursors:<30} {p:<6.3f} {n_sa:<4} {q:<6.3f} {U:<6.3f} {PUCT:<6.3f}")

        if maxindex == -1: 
            self.search_space[root].V = -0.99
            if self.debug: print(f"      [!] All branches skipped (cycle or invalid)")
            return 
        # ===================================
        ...
        for next_state in next_state_list:
            if self.is_end(next_state):
                v = 1.0
                vlist.append(v)
                if self.debug: print(f"        [EVAL] {next_state} → is_end=True → V=1.0")
                ...
                if v == -1:
                    ...
                    break
                vlist.append(v)
                if self.debug: print(f"        [EVAL] {next_state} → Value Net → V={v:.3f}")
            else:
        ...
        if sumv != -1:
            ...
            if self.debug: print(f"        [BACKUP] Child V list: {[f'{v:.3f}' for v in vlist]} → Parent V = {sumv:.3f}")
        # ===========================
        ...
        if self.debug: print(f"      [BACKUP] Selected Branch {maxindex} → N+1, V={sumv:.3f}, Q: {old_q:.3f} → {new_q:.3f}")
        # ==============
